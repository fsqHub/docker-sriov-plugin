...
1. 第一个数据包到达
   ...
   └─> 调度net_rx_action() 进行 NAPI poll 轮询
   
2. net_rx_action() 中的 NAPI poll 轮询模式
   └─> 软中断上下文中，循环调用驱动的 poll() 函数
   └─> 单个 NAPI poll 受 napi->weight 限制（常见默认 64）
   └─> 本次 net_rx_action() 还受 netdev_budget=300 和 netdev_budget_usecs 时间窗口限制
   └─> 如果网卡队列仍有数据，继续 poll
...
unsigned int netdev_budget_usecs =
    2 * USEC_PER_SEC / HZ;                       // 当前 HZ=250 时默认 8000 us
...
**budget 或时间配额耗尽后的内核行为**：

当 `budget <= 0` 或达到 `netdev_budget_usecs` 对应的 `time_limit` 时，`net_rx_action()` 会退出当前轮询循环，但不会丢弃未处理的 NAPI。内核会先递增 `sd->time_squeeze`，再把未完成的 `list`、需要再次 poll 的 `repoll`，以及执行期间新进入的 `sd->poll_list` 重新合并回当前 CPU 的 `softnet_data.poll_list`。如果该链表仍非空，就再次 raise `NET_RX_SOFTIRQ`，等待下一轮继续处理。
...
`★ Insight ─────────────────────────────────────`
- budget/time limit 耗尽是软中断让出 CPU 的机制，不等价于丢包。
- `time_squeeze` 表示本轮 `NET_RX` softirq 在预算内没处理完；持续增长说明 RX 处理能力、CPU 绑定或队列隔离需要重点排查。
- 未处理完的 NAPI 会进入下一轮 softirq；如果下一轮仍处理不完，积压会持续，最终可能表现为延迟升高、`ksoftirqd` 占用升高或队列满后的丢包。
`─────────────────────────────────────────────────`
...
2. **共享 NAPI Budget**：
   ...
   - `netdev_budget = 300` 是每次 `net_rx_action()` 的总包数配额，在本 CPU 本轮待 poll 的 NAPI 实例间消耗
   - `netdev_budget_usecs` 是每次 `net_rx_action()` 的总时间窗口，不是单个 NAPI poll 的时间配额
   - IPvlan 容器共享父设备 NAPI 路径；竞争发生在同 CPU、同父设备队列和同一软中断窗口内，不按容器公平切分 CPU 时间片
...
|------|--------------|-------------------|--------------|
| **NAPI 实例数量** | IPvlan 虚拟设备自身 0；RX 复用父设备 PF 的 NAPI | N（PF 的所有 NAPI 实例） | M（VF 的独立 NAPI 实例，M 通常 < N） |
...
static void net_rx_action(struct softirq_action *h)
{
    ...
    for (;;) {
        ...
        // ★ 如果包数 budget 耗尽或软中断时间窗口耗尽，退出本轮软中断处理
        if (budget <= 0 || time_after_eq(jiffies, time_limit))
            break;
    }
}
...
✅ 优势：如果 VF 中断与队列被分散绑定到不同 CPU，各 CPU 会分别消耗本次 net_rx_action() 的包数和时间 budget
```

在这个过程中：
- **IPvlan 模式**：如果 3 个 NAPI 实例处理的数据包分别属于容器 A、B、C，则它们在同一个 CPU 的 softnet_data 中共享本轮 `net_rx_action()` 的 300 包数配额和 `netdev_budget_usecs` 时间窗口
- **VF 直通模式**：如果各 VF 中断绑定到不同 CPU，各 CPU 有独立的 softnet_data 和本次软中断包数/时间 budget；若 IRQ/RPS/队列绑定重叠，仍可能竞争
...
# 单次 NET_RX_SOFTIRQ/net_rx_action() 的最大处理时间（单位：微秒）
# 默认值 = 2 * USEC_PER_SEC / HZ；当前内核 CONFIG_HZ=250，因此默认 8000
sysctl net.core.netdev_budget_usecs

# skb 时间戳预队列开关（默认 1）：控制 skb 在进入 backlog 队列前后打时间戳
# 不是软中断处理时间参数
sysctl net.core.netdev_tstamp_prequeue
```

#### 1.8.2 NAPI weight、netdev_budget 与 netdev_budget_usecs 的区别
...
| **netdev_budget_usecs** | 单次软中断（net_rx_action）的总时间配额 | `2 * USEC_PER_SEC / HZ`；当前 HZ=250 时为 8000 us | 可通过 sysctl 调整，受最小值限制 |
| **netdev_tstamp_prequeue** | skb 时间戳预队列开关 | 1 | 可通过 sysctl 调整 |
...
static void net_rx_action(struct softirq_action *h)
{
    ...
    for (;;) {
        ...
        budget -= work;                      // 从本轮 net_rx_action 包数配额扣除实际处理数
...
- `netdev_budget` 决定单次 `net_rx_action()` 最多处理多少个包；它是全局变量，但在每个 CPU 的软中断执行中读取并独立消耗
- `netdev_budget_usecs` 决定单次 `net_rx_action()` 的总时间窗口，不是每个 NAPI poll 的时间配额
- 当前内核默认值计算：`2 * 1,000,000 / CONFIG_HZ(250) = 8000 us`
- `netdev_tstamp_prequeue` 只影响 skb 时间戳打点位置，不限制软中断时长
...
- 调整 `netdev_budget` 和 `netdev_budget_usecs` 比调整 `weight` 更简单；前者影响本轮软中断吞吐，后者影响软中断占用 CPU 的最长窗口，二者都需要压测验证延迟和吞吐变化
...
1️⃣ Docker IPvlan 模式
   ...
   │
   └── netdev_budget=300 / netdev_budget_usecs=8000us
       ←─ 本 CPU 本轮 net_rx_action() 的包数/时间窗口，父设备 NAPI 共享消耗
...
2️⃣ Docker Host Network 模式
   ...
   │
   └── netdev_budget=300 / netdev_budget_usecs=8000us
       ←─ 本 CPU 本轮 net_rx_action() 与宿主机和 Host Network 容器流量共同消耗
...
3️⃣ Docker VF 直通模式
   ...
   │
   └── CPU0 的 softnet_data 独立消耗本轮 netdev_budget=300 / netdev_budget_usecs=8000us
   ...
   │
   └── CPU1 的 softnet_data 独立消耗本轮 netdev_budget=300 / netdev_budget_usecs=8000us

   ✅ 隔离条件：
      - netdev_budget=300 和 netdev_budget_usecs=8000us 是全局变量，但每个 CPU 执行 net_rx_action() 时读取并独立消耗
      - VF0 的中断 → CPU0 → CPU0 的 softnet_data.poll_list → CPU0 独立消耗本轮包数/时间 budget
      - VF1 的中断 → CPU1 → CPU1 的 softnet_data.poll_list → CPU1 独立消耗本轮包数/时间 budget
...
3. **IPvlan 模式**中，所有容器共享 PF 的 NAPI 实例；同 CPU 同软中断窗口内会共同消耗 `netdev_budget` 与 `netdev_budget_usecs`，高并发时存在间接竞争
4. **Host Network 模式**中，容器复用宿主机物理网卡 NAPI 路径，无虚拟设备路径开销，但不独占 NAPI 资源
5. **NAPI weight** 决定单个 NAPI poll 的最大数据包数（默认 64），**netdev_budget** 决定单次软中断处理的总数据包数（默认 300），**netdev_budget_usecs** 决定单次软中断处理的总时间窗口（当前内核默认 8000 us）
...
```
1. 网卡 DMA 完成
   └─> 网卡驱动构建 skb：如 napi_build_skb()/napi_alloc_skb()，取决于驱动和 page/page_pool 模式
   └─> skb->data 指向驱动整理后的数据区域
...
| 操作 | 函数 | 说明 | 是否拷贝数据 |
|------|------|------|-------------|
| **分配/构建 skb** | `alloc_skb()` / `napi_alloc_skb()` / `napi_build_skb()` | 通用 skb 分配或 RX 路径从已有 buffer 构建 skb，具体取决于驱动和模式 | - |
...
| **修改 metadata** | 修改 `skb->dev`、协议字段、MAC header 等 | `dev_forward_skb()` 会清理/重置部分 metadata，并通过 `netif_rx_internal()` 注入目标 netdev | ✗ |
...
|------|---------------------|-------------|
| **IPvlan** | 1. PF 驱动构建 skb<br>2. `rx_handler` 拦截，按模式修改 `skb->dev` 或转发 skb<br>3. `dev_forward_skb()` 经过 `__dev_forward_skb()` 清理 metadata、设置协议，再由 `netif_rx_internal()` 注入目标设备<br>4. 进入容器 netns，继续 L3/L4 处理 | 通常不做整包数据拷贝，主要传递 skb 指针并调整 metadata |
| **Host Network** | 1. PF 驱动构建 skb<br>2. 直接在宿主机 netns 中完成 L2→L4 处理<br>3. 挂到 socket 接收队列 | 内核协议栈内通常不做整包数据拷贝 |
| **VF 直通** | 1. VF 驱动构建 skb（DMA 映射受 IOMMU/直通 DMA 配置影响）<br>2. skb 进入 VF netdev 所在 netns 的协议栈；IRQ/NAPI 仍由宿主机内核调度<br>3. 挂到 socket 接收队列 | 内核协议栈内通常不做整包数据拷贝 |

### 2.7 关键结论

1. **三种模式在内核协议栈中主要通过 skb 指针传递**，容器网络转发通常不会做整包数据拷贝
2. IPvlan 的 `dev_forward_skb()` 路径会清理/重置 metadata、设置 `skb->protocol`，再通过 `netif_rx_internal()` 注入目标设备；它不是简单地只改一个 `skb->dev`
3. 最常见、最明确的数据拷贝发生在用户态边界：
   - RX：普通 `read()`/`recvmsg()`（skb → 用户态 buffer）
   - TX：普通 `write()`/`sendmsg()`（用户态 buffer → skb）
   - 但内核路径中仍可能因 `skb_clone()`、`skb_copy()`、linearize、GRO/XDP、header copy 或 offload 限制产生额外拷贝/重组；zero-copy 场景则另当别论
...
软中断处理（NET_RX_SOFTIRQ）
    ...
    └─> napi->poll()  // 驱动的 poll 函数
        └─> ixgbe_clean_rx_irq()
            └─> 从 RX 环读取描述符
            └─> napi_build_skb()/napi_alloc_skb()  // 构建 skb，具体取决于驱动路径
            └─> skb->data 指向驱动整理后的数据区域
...
- `dma_map_page_attrs()` 返回 `dma_addr_t`。启用 IOMMU 时通常是 IOVA；未启用或直通映射时可能接近/等同总线地址，不能简单等同为 CPU 物理地址
- 在启用 IOMMU 隔离的 VF 直通场景中，VF 硬件只能访问 IOMMU 映射允许的 DMA 地址范围，实现地址隔离

### 4.4 硬件队列（RSS Multi-Queue）

**RSS（Receive Side Scaling）** 是网卡的多队列技术，将不同的流（flow）分配到不同的硬件队列。

**ixgbe 驱动写入 RSS RETA**（`drivers/net/ethernet/intel/ixgbe/ixgbe_main.c:3798`）：
...
| 组件 | 源码路径 | 关键函数/结构体 |
|------|---------|---------------|
| **NAPI 核心** | `net/core/dev.c` | `net_rx_action()`, `__napi_poll()`, `netdev_budget`, `netdev_budget_usecs`, `netdev_tstamp_prequeue` |
