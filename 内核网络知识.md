...
```c
// IPvlan 设备创建时只注册 rx_handler，不创建 NAPI 实例
int ipvlan_link_new(struct net *src_net, struct net_device *dev, ...)
{
    // ...
    // 注册 rx_handler 拦截父设备的数据包，但不调用 netif_napi_add()
...
3. **数据包分发时机**：在 NAPI poll 完成后，通过 `rx_handler` 在软中断上下文中分发到各容器
...
1. **独占 NAPI 实例**：容器独占物理网卡的所有 NAPI 实例
2. **独占 NAPI Budget**：无需与其他容器竞争 `netdev_budget`
3. **无额外开销**：没有 `rx_handler` 拦截、没有 netns 切换

#### 1.7.3 Docker VF 直通模式

**NAPI 实例归属**：每个 VF 有**独立的 NAPI 实例**，在容器 netns 中运行。
...
1. VF 硬件中断（通过 VFIO/MSI-X 直接路由到容器）
   └─> VF 驱动的 napi_schedule(&q_vector->napi)  // VF 独立的 NAPI 实例
   
2. 软中断上下文：net_rx_action()（在容器的 CPU 上执行）
   └─> 遍历容器 CPU 的 poll_list，找到 VF 的 napi_struct
   └─> 调用 VF 的 poll() 函数：ixgbevf_poll()
       └─> 从 VF 的硬件队列读取数据包
       └─> netif_receive_skb()
           └─> **直接在容器 netns 中完成协议栈处理**
...
1. **独立 NAPI 实例**：
   - 每个 VF 有自己的 `napi_struct`，通过 `netif_napi_add()` 注册到容器 netns
   - `napi->weight` 默认为 `NAPI_POLL_WEIGHT = 64`（与 PF 相同）
   
2. **独立 NAPI Budget（通过 per-CPU 隔离）**：
   - **关键机制**：VF 的 NAPI 实例注册到容器 netns，中断被绑定到容器的专属 CPU
   ...
   - **源码证明**（`net/core/dev.c`）：
     ...
     static void net_rx_action(struct softirq_action *h)
     {
         ...
         // 遍历当前 CPU 的 poll_list（只包含绑定到此 CPU 的 NAPI 实例）
         for (;;) {
     ...
     ```
   - **实际效果**：容器 A 的 VF 中断绑定到 CPU0，容器 B 的 VF 中断绑定到 CPU1，它们各自消耗独立的 300 budget
   
3. **独立硬件队列**：
   - VF 有独立的 RX/TX 描述符环（Ring Buffer）
   - VF 产生独立的 MSI-X 中断，通过 IRQ 亲和性绑定到容器的专属 CPU
...
| **NAPI 实例位置** | 宿主机 netns（父设备） | 宿主机 netns（容器共享） | 容器 netns（隔离） |
| **单个 NAPI weight** | 继承父设备（通常 64） | PF 默认 64 | VF 默认 64 |
| **softnet_data** | 共享宿主机 CPU 的 softnet_data | 独占宿主机 CPU 的 softnet_data | **每个容器的 VF 中断绑定到独立 CPU**，各 CPU 的 softnet_data 隔离 |
| **netdev_budget 隔离** | **间接竞争**（父设备 NAPI 被多个容器占用） | 无竞争 | **通过 per-CPU 隔离**（VF 中断绑定到不同 CPU，各 CPU 独立消耗 300 budget） |
...
3. **VF 直通模式的 Budget 隔离机制**：
   - VF 的 MSI-X 中断通过 IRQ 亲和性绑定到容器的专属 CPU（如 CPU0）
   - 软中断 `net_rx_action()` 在该 CPU 上执行，访问 `this_cpu_ptr(&softnet_data)`
   - 该 CPU 的 `poll_list` 只包含该容器 VF 的 NAPI 实例
   - 该 CPU 独立消耗 `netdev_budget=300`，与其他 CPU 无竞争
...
✅ 优势：每个 CPU 独立消耗 netdev_budget=300，容器之间完全隔离
```

在这个过程中：
- **IPvlan 模式**：如果 3 个 NAPI 实例处理的数据包分别属于容器 A、B、C，则它们在同一个 CPU 的 softnet_data 中共享 300 budget
- **VF 直通模式**：每个容器的 VF 中断绑定到不同 CPU，各 CPU 有独立的 softnet_data 和独立的 300 budget 消耗，无竞争
...
2️⃣ Docker Host Network 模式
   宿主机 netns (容器共享)
   ├── 物理网卡 (PF)
   │   ├── NAPI 实例 1 (queue 0, weight=64)  ←─ 容器独占
   │   ├── NAPI 实例 2 (queue 1, weight=64)  ←─ 容器独占
   │   └── NAPI 实例 N (queue N-1, weight=64) ←─ 容器独占
   │
   └── 全局 netdev_budget = 300  ←─ 容器独占
...
3️⃣ Docker VF 直通模式
   ...
   容器 A netns (VF0 中断绑定到 CPU0)
   ├── VF 0 (SR-IOV Virtual Function)
   │   ├── NAPI 实例 1 (VF queue 0, weight=64)  ←─ 注册到 CPU0 的 softnet_data.poll_list
   │   └── NAPI 实例 2 (VF queue 1, weight=64)  ←─ 注册到 CPU0 的 softnet_data.poll_list
   │
   └── CPU0 的 softnet_data 独立消耗 netdev_budget=300

   容器 B netns (VF1 中断绑定到 CPU1)
   ├── VF 1 (SR-IOV Virtual Function)
   │   ├── NAPI 实例 1 (VF queue 0, weight=64)  ←─ 注册到 CPU1 的 softnet_data.poll_list
   │   └── NAPI 实例 2 (VF queue 1, weight=64)  ←─ 注册到 CPU1 的 softnet_data.poll_list
   │
   ...
   ✅ 无瓶颈：
      ...
      - 容器之间通过 CPU 亲和性实现完全隔离
```

1. **NAPI 是 Linux 高性能网络的基石**，通过中断 + 轮询混合模式平衡延迟和吞吐量

2. **VF 直通模式**中，每个容器有独立的 NAPI 实例和 budget，完全隔离，无竞争
3. **IPvlan 模式**中，所有容器共享 PF 的 NAPI 实例和全局 budget，高并发时存在间接竞争
4. **Host Network 模式**中，容器独占物理网卡的所有 NAPI 资源，性能与物理机一致
...
| **rx_handler** | `net/core/dev.c:__netif_receive_skb_core()` | `rx_handler` 调用点 |
...
2. **rx_handler 调用点**：`net/core/dev.c:__netif_receive_skb_core()`
   - 在协议栈处理前拦截数据包
   - IPvlan 通过此机制实现数据包分发
...
6. CPU0 独立消耗 300 budget，与 CPU1 的 VF 完全隔离
```
