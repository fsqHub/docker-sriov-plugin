...
![三种网络模式对比](diagrams/network_modes_comparison.png)
...
### Linux 内核网络栈中的 skb（socket buffer）

在深入理解三种网络模式的数据流之前，需要先了解 Linux 内核网络栈的核心数据结构 **skb**。

#### 什么是 skb

`struct sk_buff`（简称 skb）是 Linux 内核中网络数据包的统一表示，贯穿整个网络栈（从网卡驱动到应用层 socket）。

**核心字段**（`include/linux/skbuff.h`）：
...
**接收路径（RX）**：
```
网卡 DMA → 驱动分配 skb → IP 层处理（skb->data 后移） → TCP 层处理 
→ 挂到 socket 接收队列 → read() 拷贝到用户态 → 释放 skb
```

**发送路径（TX）**：
```
write() 分配 skb → TCP 层添加头（skb->data 前移） → IP 层添加头 
→ 驱动发送 → DMA 传输 → 释放 skb
```
...
| 模式 | skb 处理方式 | 关键操作 |
|------|-------------|---------|
| **IPvlan** | `rx_handler` 拦截后，调用 `skb_clone()` 或直接修改 `skb->dev` 指向 IPvlan 虚拟设备 | `dev_forward_skb()` 只修改 metadata，**不拷贝数据** |
| **Host Network** | PF 驱动分配的 skb 直接在宿主机 netns 中处理 | 标准内核路径 |
| **VF 直通** | VF 驱动分配的 skb 直接在容器 netns 中处理（DMA 通过 IOMMU） | 与 Host Network 相同的处理流程 |

**关键概念**：
- `skb_clone()`：只复制 skb 结构体，共享底层数据区域（引用计数机制）
- `skb_copy()`：深拷贝，包括结构体和数据区域（IPvlan 通常不使用）
- **metadata 调整**：`dev_forward_skb()` 修改 `skb->dev`、重置 MAC header，但 `skb->data` 指向的数据区域保持不变

**结论**：三种模式在内核协议栈中都是**通过 skb 指针传递**，不拷贝数据。真正的数据拷贝只发生在用户态边界（`read()`/`write()` 系统调用）。
...
**关键特征**：
- **零额外开销**：数据包直接进入容器进程，网络栈路径等同于主机普通进程
...
VF 通过 IOMMU 获得独立的 DMA 地址空间，实现硬件级内存隔离。

**① IOMMU Domain 分配（每个 VF 独立）**
每个 VF 有独立的 IOMMU domain。

**代码位置**: `drivers/iommu/iommu.c:1891-1909` 和 `drivers/pci/pci.c:1641-1662`

```c
// VF PCI 设备探测时，内核自动为其分配独立的 IOMMU domain
// drivers/iommu/iommu.c:1891
struct iommu_domain *iommu_get_dma_domain(struct device *dev)
{
    return dev->iommu->default_domain;  // 每个 VF 有独立的 default_domain
}

// PCI 设备初始化时创建 IOMMU group（VF 通过 BDF 唯一识别）
// drivers/pci/pci.c:1641
static int pci_dma_configure(struct device *dev)
{
    struct device *bridge = pci_get_host_bridge_device(to_pci_dev(dev));
    
    // 为 VF 创建独立的 IOMMU group
    ret = of_dma_configure(dev, bridge->of_node, true);
    
    // VF 的 DMA 操作受 IOMMU 约束，只能访问映射的地址
...
VF 的 DMA 映射（dma_map_page_attrs）通过 IOMMU 转换，只能访问分配给它的内存区域。

**代码位置**: `kernel/dma/mapping.c:341-358`

```c
// VF 驱动调用 dma_map_page_attrs 时，IOMMU 介入地址转换
// kernel/dma/mapping.c:341
dma_addr_t dma_map_page_attrs(struct device *dev, struct page *page,
                   size_t offset, size_t size,
                   enum dma_data_direction dir, unsigned long attrs)
{
    const struct dma_map_ops *ops = get_dma_ops(dev);  // ← 获取 IOMMU 的 DMA 操作函数
    dma_addr_t addr;
    
    // 调用 IOMMU 驱动的 map_page 函数（如 intel_iommu_map_page）
    addr = ops->map_page(dev, page, offset, size, dir, attrs);
    
    // 返回的是 IOVA（IO Virtual Address），不是物理地址
    // VF 硬件使用 IOVA 访问内存，IOMMU 硬件负责 IOVA → 物理地址转换
    return addr;
}
```
...
**代码位置**: `drivers/iommu/intel/iommu.c:2156-2189` (Intel IOMMU 示例)

```c
// 每个 VF 有独立的 IOMMU 页表
// drivers/iommu/intel/iommu.c:2156
               ...
               size_t size, int iommu_prot, gfp_t gfp)
{
    ...
    // 写入该 VF 专属的 IOMMU 页表
    // 其他 VF 的 IOMMU 页表中不包含此映射，无法访问
    ret = domain_mapping(dmar_domain, iova, hpa, size, prot);
    
    return ret;
}

// IOMMU 页表结构（类似 CPU 页表，但用于 DMA 地址转换）
// VF #1 的 IOMMU 页表: IOVA_1 → 物理地址 A
// VF #2 的 IOMMU 页表: IOVA_1 → 物理地址 B（完全独立）
```

**④ VF DMA 访问时 IOMMU 硬件强制检查**

**硬件行为**（IOMMU 规范，非软件代码）：

```
VF 硬件发起 DMA 请求（使用 IOVA）
    ↓
IOMMU 硬件拦截（根据 VF 的 BDF 识别）
    ↓
查找该 VF 的 IOMMU 页表
    ↓
情况 1：IOVA 在页表中 → 转换为物理地址，允许访问
情况 2：IOVA 不在页表中 → **硬件拒绝，触发 IOMMU fault**
    ↓
CPU 收到 IOMMU fault 中断，内核打印错误日志：
"DMAR: DRHD: handling fault status reg X"
"DMAR: [DMA Write] Request device [XX:XX.X] fault addr XXXXXXXX"
```

**⑤ VF 之间隔离的源码证据链**

**代码位置**: `drivers/pci/iov.c:156-188` (SR-IOV VF 分配)

```c
// 每个 VF 被分配唯一的 BDF（Bus:Device.Function）
// drivers/pci/iov.c:156
static int sriov_enable(struct pci_dev *dev, int nr_virtfn)
{
    // 为每个 VF 分配独立的 PCI 配置空间
    for (i = 0; i < nr_virtfn; i++) {
        // VF 的 BDF = PF BDF + offset + i
        vf_bdf = pci_iov_virtfn_bdf(dev, i);
        
        // 创建 VF 的 pci_dev 结构，内核视为独立设备
        virtfn = pci_get_dev_by_id(vf_bdf);
        
        // 为 VF 创建独立的 IOMMU group（根据 BDF）
        // → 每个 VF 有独立的 IOMMU domain
        // → 每个 VF 有独立的 IOMMU 页表
        iommu_group_get(&virtfn->dev);
    }
}
```

**关键证明**：
1. **每个 VF 有唯一的 BDF** → IOMMU 硬件根据 BDF 识别 DMA 请求来源
2. **每个 VF 有独立的 IOMMU domain** → 独立的页表，映射关系完全隔离
3. **VF 的 DMA 地址是 IOVA（IO Virtual Address）** → 不是物理地址，必须通过 IOMMU 转换
4. **IOMMU 硬件强制检查** → 访问未映射地址时触发 fault，无法绕过

**IOMMU 隔离与 CPU 虚拟内存的类比**：
```
CPU 虚拟内存隔离（进程级）    |  IOMMU 地址隔离（VF 级）
进程 A 的页表: VA → PA_A       |  VF #1 的 IOMMU 页表: IOVA → PA_A
进程 B 的页表: VA → PA_B       |  VF #2 的 IOMMU 页表: IOVA → PA_B
进程 A 无法访问 PA_B          |  VF #1 无法访问 PA_B
硬件 MMU 强制隔离             |  硬件 IOMMU 强制隔离
```

**实测验证（IOMMU fault 日志）**：

如果 VF 驱动存在 bug，尝试访问未映射的地址，会触发：
```bash
# dmesg | grep DMAR
[  123.456789] DMAR: DRHD: handling fault status reg 2
[  123.456790] DMAR: [DMA Write] Request device [04:10.0] fault addr 0xdeadbeef
[  123.456791] DMAR: fault reason 0x06 (PTE Write access is not set)
```

这证明 IOMMU 在**硬件层面强制隔离**，而非软件策略。
...
**关键点**：整个数据路径中，**宿主机**参与的仅限于 **VF 分配和配置阶段**，数据传输阶段完全在 **VF 硬件与容器netns之间完成**，不经过宿主机的netns。
...
![SR-IOV硬件架构](diagrams/sriov_hardware_architecture.png)
...
- **VF 略 < PF**（理论）：
  ...
  - 现代 IOMMU（Intel VT-d、AMD-Vi）性能开销较低，但具体取决于硬件、IOTLB 大小和 DMA 映射模式
...
#### 负面因素：
1. **IOMMU 地址转换开销**：可能存在性能损失（依赖硬件和配置，需实测确认）
